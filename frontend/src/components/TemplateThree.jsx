...
import { CertificationInfo } from "./ResumeSection";
...
const TemplateThree = ({ resumeData = {}, containerWidth }) => {
  ...
  return (
    ...
    >
      {/* Header Section */}
      <header className="px-8 pt-8 pb-4 mb-2 text-center">
      ...
      <div className="grid grid-cols-12 gap-4 px-8 pb-8">
        ...
        <aside className="col-span-5 space-y-5 pr-4 border-r border-gray-300 text-left">
          {/* Contact */}
          <section>
            ...
            <ul className="text-xs text-gray-700 space-y-2 pb-2">
              {contactInfo.location && (
                <li className="flex items-start">
                  <span className="font-semibold min-w-[65px]">Location:</span> {contactInfo.location}
                </li>
              )}
              {contactInfo.phone && (
                <li className="flex items-start">
                  <span className="font-semibold min-w-[65px]">Phone:</span> {contactInfo.phone}
                </li>
              )}
              {contactInfo.email && (
                <li className="flex items-start">
        ...
        <main className="col-span-7 space-y-5 pl-4 text-left">
          {/* Work Experience */}
            ...
            <section>
              ...
              <div className="space-y-5">
                {workExperience.map((exp, idx) => (
                  <div key={idx} className="text-xs" style={{ breakInside: 'avoid', pageBreakInside: 'avoid', WebkitColumnBreakInside: 'avoid' }}>
                    <h3 className="font-bold pb-1">{exp.role}</h3>
                    <p className="italic">{exp.company}{exp.location && `, ${exp.location}`}</p>
                    {exp.startDate && exp.endDate && (
                      <p className="italic text-[11px]">{formatYearMonth(exp.startDate)} – {formatYearMonth(exp.endDate)}</p>
                    )}
                ...
                {projects.map((proj, idx) => (
                  <div key={idx} className="text-xs">
                    ...
                    {proj.startDate && proj.endDate && (
                      <p className="italic text-[11px]">{formatYearMonth(proj.startDate)} – {formatYearMonth(proj.endDate)}</p>
                    )}
